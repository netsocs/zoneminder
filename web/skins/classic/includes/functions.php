...
function getRamHTML()
{
  ...
  $meminfo = array_combine($matches[1], array_map(function ($v) {
    return 1024 * $v;
  }, $matches[2]));
...
function getNavBrandHTML()
{
  ...
  if (ZM_HOME_ABOUT) {
    $result .= '<a id="getNavBrandHTML" class="dropdown" data-toggle="dropdown" href="#">Netsocs</a>' . PHP_EOL;
    $result .= '<ul style="background-color:#485460" class="dropdown-menu">' . PHP_EOL;
    $result .= '<li><a class="dropdown-item" href="https://zoneminder.com/" target="_blank">Netsocs</a></li>' . PHP_EOL;
...
function getAccountCircleHTML($skin, $user = null)
{
  ...
  if (ZM_OPT_USE_AUTH and $user) {
    ...
    $result .= makeLink(
      '#',
      '<i class="material-icons">account_circle</i> ' . validHtmlStr($user['Username']),
      (ZM_AUTH_TYPE == 'builtin'),
      'id="logoutButton" data-toggle="modal" data-target="#modalLogout" data-backdrop="false"'
    ) . PHP_EOL;
    $result .= '</li>' . PHP_EOL;
  }
